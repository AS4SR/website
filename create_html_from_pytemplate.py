...
#
def create_and_return_verticalmenubar(html_create_list):
...
#        verticalmenubar_part.append(" "*(tabs*4+2) + \
#            """<li class="%s"><a href='%s%s%s' _target="%s">%s</a>""" % #(html_create_list[i][1],website_top,html_create_list[i][2],html_create_list[i][3],thetarget,html_create_list[i][5]))
        i += 1
...
def create_css_file_and_write_to_disk(gitdir,website_top,css_to_use):
...
def create_html_file_and_write_to_disk(html_top,html_create_list,full_templatedir,html_full_template,titlerider,website_top,css_filedir,css_filename,gitdir,i,local_compile_check):
    ...
    html_replace_dict.update( {"PAGETITLE": html_create_list[i][5] + titlerider} )
    ...
    html_replace_dict.update( {"BODYCLASS": html_create_list[i][1]} )
    ...
    verticalmenubar_part_str = create_and_return_verticalmenubar(html_create_list)
...
if __name__ == '__main__':
    ...
    if (True):
        ...
        #
        create_css_file_and_write_to_disk(gitdir,website_top,css_to_use)
        ...
        for i in range(len(html_create_list)):
            create_html_file_and_write_to_disk(html_top, html_create_list, \
                full_templatedir, html_full_template, \
                titlerider, \
                website_top, \
                css_filedir, css_filename, \
                gitdir, i, local_compile_check)
